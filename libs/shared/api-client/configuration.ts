...
export interface PomsApiConfigurationParameters {
    /**
...
export class PomsApiConfiguration {
    /**
     *  @deprecated Since 5.0. Use credentials instead
     */
    apiKeys?: {[ key: string ]: string};
...
constructor({ accessToken, apiKeys, basePath, credentials, encodeParam, encoder, password, username, withCredentials }: PomsApiConfigurationParameters = {}) {
        if (apiKeys) {
     ...
     * Uses {@link PomsApiConfiguration#isJsonMime} to determine the correct content-type.
     ...
     * Uses {@link PomsApiConfiguration#isJsonMime} to determine the correct accept content-type.
     * If no content type is found return the first found type if the contentTypes is not empty
