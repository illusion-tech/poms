...
| `ProjectDetailView`            | `Project`         | 支撑项目详情页               | 主体字段、销售主责、阶段摘要、当前投标摘要、当前合同摘要、当前审批 / 确认摘要、`summarySnapshotId`、`projectionLevel`、`allowedActions`                                                                                                                     | 总览卡片中的审批摘要必须回到当前场景的 `summarySnapshotId / projectionLevel / exportPolicy`；销售主责变更入口由 `allowedActions` 返回 `reassign-project-owner` |
| `ProjectWorkspaceGuidanceView` | `Project`         | 支撑项目工作区首页连续引导   | `projectId`、`currentStage/status`、业务中文阶段 / 状态标签、`headline`、`currentFocus`、`currentGap`、`nextStep`、`ownerLabel`、`blockingReasons`、`basisSummary`、`recommendedEntries`、`generatedAt`                                                     | 不代替 `ProjectDetailView`；前端不得再用 `stage/status` 本地拼下一步、当前缺口或责任归口                                                                       |
...
| `ProjectActualCostRecordListView`   | `ProjectActualCostRecord`       | 支撑项目实际成本列表与来源筛选        | `costType`、`occurredOn`、`executionStageCode`、`amountIncludingTax`、`recordStatus`、`isIncludedInProjectCost`、`sourceType`、`sourceRefNo`、`evidenceSummary`                                                                                        | 默认返回项目级摘要，不默认展开敏感明细；需支持按 `costType / recordStatus / sourceType` 筛选       |
| `ProjectActualCostRecordDetailView` | `ProjectActualCostRecord`       | 支撑付款事实 / 人力成本归集与替代追溯 | `costType`、`sourceType`、`sourceId`、`sourceRefNo`、来源当前状态摘要、`rateVersionId`、生效区间、计量依据摘要、替代关系摘要、`allowedActions`                                                                                                         | `PAYMENT_FACT` 必须可回看付款事实；`LABOR` 详情默认不向无权角色下钻人员级成本率原值                |
