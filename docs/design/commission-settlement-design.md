...
- 第二阶段与最终阶段发放统一复用 `AcceptanceRecord` 作为确认对象，不再额外定义新的“阶段成果确认对象”
- 第二阶段当前只认“阶段成果确认 / 阶段验收”类型的有效 `AcceptanceRecord`
- 有效 `AcceptanceRecord` 至少应包含：确认结论、确认人、确认时间、阶段成果范围摘要，以及至少一项证据引用
...
- 系统计算可发放上限，审批确定采用哪一档
- `AcceptanceRecord` 必须能对应本次第二阶段发放所依据的阶段成果范围
- 若证据链不完整，或当前记录仅为最终验收而非第二阶段阶段成果确认，不得直接替代第二阶段前置条件
- 第二阶段发放申请应显式引用被消费的 `AcceptanceRecord`
