...
| Domain       | Capability                          | Canonical Route                                 | Current Implemented Route                                                                                                        | Current Design Route                            | Authority                                     | Drift Type                                       | Action                                                                                       | Batch | Status                 |
| ------------ | ----------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | --------------------------------------------- | ------------------------------------------------ | -------------------------------------------------------------------------------------------- | ----- | ---------------------- |
| `commission` | `getCommissionRoleAssignment`       | `GET /commission-role-assignments/{id}`         | `GET /commission-role-assignments/:id/detail`                                                                                    | `GET /commission-role-assignments/{id}`         | `ADR-015` + `EX-09` route governance          | `page-suffix-query`                              | 在 `EX-15D` 去掉 `/detail`，统一 item detail canonical path                                  | `B1`  | `implementation-drift` |
| `commission` | `freezeCommissionRoleAssignment`    | `POST /commission-role-assignments/{id}:freeze` | `POST /commission-role-assignments/:id/freeze`; legacy alias = `POST /commission/projects/:projectId/role-assignment/:id/freeze` | `POST /commission-role-assignments/{id}:freeze` | `ADR-015` + `interface-openapi-dto-design.md` | `slash-vs-colon-action` + `legacy-alias-present` | 直接切到 colon-action，并下线 project-scoped legacy freeze alias，除非出现不可控外部依赖例外 | `B1`  | `implementation-drift` |
| `commission` | `createCommissionRoleChangeRequest` | `POST /commission-role-change-requests`         | N/A                                                                                                                              | `POST /commission-role-change-requests`         | `ADR-015` + `interface-openapi-dto-design.md` | `resourceization-required`                       | 直接按独立资源落 EX-09D，不再实现 `:submitChange` canonical 路由                             | `B1`  | `planned`              |
| `commission` | `getCommissionRoleChangeRequest`    | `GET /commission-role-change-requests/{id}`     | N/A                                                                                                                              | `GET /commission-role-change-requests/{id}`     | `ADR-015` + `EX-09` route governance          | `resourceization-required`                       | 作为 EX-09D 读侧能力同步建模，不再把变更申请详情塞回 assignment action 语义                  | `B1`  | `planned`              |
