...
| `commission` | `createCommissionRoleChangeRequest` | `POST /commission-role-change-requests`         | N/A                                            | `POST /commission-role-change-requests`         | `ADR-015` + `interface-openapi-dto-design.md` | `resourceization-required` | `EX-09D` 先裁决 “direct-request” 与 “dispute-first” 正式输入冲突，再实现独立资源           | `B1`  | `planned` |
| `commission` | `getCommissionRoleChangeRequest`    | `GET /commission-role-change-requests/{id}`     | N/A                                            | `GET /commission-role-change-requests/{id}`     | `ADR-015` + `EX-09` route governance          | `resourceization-required` | 读侧与持久化必须跟随 `EX-09D` 的 dispute-first 基线一起冻结，不能先猜 query 形态           | `B1`  | `planned` |
