...
第一阶段角色与权限补齐时，至少应落到以下能力：

- 权限字典的只读查询与后端受控初始化
- 角色列表、详情、创建、更新、启停
- 角色权限分配与撤销
- 用户有效权限的真实后端计算
- 导航过滤与接口鉴权统一消费同一权限事实源
...
本模块属于 `P1-S08` 与 `P1-S09` 的共同输入。

补齐顺序建议：

1. 先保持 `Permission` 为共享常量 + 后端种子模式
2. 补齐 `Role` 与 `RolePermissionAssignment` 的持久化与管理接口
3. 再让导航过滤与当前用户权限计算切换到真实角色关系结果
