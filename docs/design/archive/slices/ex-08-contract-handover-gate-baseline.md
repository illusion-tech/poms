...
| Route / query / command naming | `interface-openapi-dto-design.md`                                                        | `GET /projects/{projectId}/contract-handover`、`GET /projects/{projectId}/project-handover`、`GET /project-handovers/{id}`、`POST /project-handovers/{id}:confirm` 与 `POST /contract-handover-rebaselines` 共同构成本切片 canonical route |
...
| --------------------------------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /project-handovers/{id}:confirm`              | `confirmProjectHandover`          | `comment`、`participantConfirmations`、`receiptJudgmentMode`、`contractSummarySnapshotId`、`expectedVersion`          | `targetId`、`businessStatusAfter`、`confirmationRecordId`、`receiptJudgmentFreezeId`、`contractSummarySnapshotId`、`effectiveHandoverBaselineSnapshotId`、`summarySnapshotId`、`projectionLevel`、`exportPolicy` | 合同已生效、摘要快照存在、参与人齐备、执行负责人明确、无未收口再基线化、版本匹配 | `interface-openapi-dto-design.md` §5.1  | EX-08B3A 已实现确认主链；`EX-15C` 已将运行时路由收口为 `/api/project-handovers/{handoverId}:confirm` 并通过 build / test / openapi / e2e 验证 |
| `POST /contract-handover-rebaselines`               | `rebaselineContractHandover`      | `contractAmendmentId`、`rebaselineReason`、`affectedHandoverItemIds[]`、`effectiveBaselineAfterId`、`expectedVersion` | `targetId`、`rebaselineRecordId`、`effectiveBaselineAfterId`、`resultStatus`                                                                                                                                     | 已存在生效合同变更、移交前承接事实已形成、影响范围明确、当前状态允许再基线化     | `interface-openapi-dto-design.md` §5.5B | EX-08B3B1 已实现；设计与当前实现已对齐，不实现合同变更审批流                                                                                  |
| `POST /approval-summary-packages/{targetId}:review` | `reviewApprovalSummaryProjection` | `approvalScenarioKey`、`summaryPackageKey`、`projectionLevel`、`exportPolicy`、`comment`、`expectedVersion`           | `targetId`、`approvalScenarioKey`、`summaryPackageKey`、`summarySnapshotId`、`projectionLevel`、`exportPolicy`、`resultStatus`                                                                                   | 场景和字段包定义存在，调用人具备复核 / 生成权限                                  | `interface-openapi-dto-design.md` §5.5B | 最小实现归入 EX-08A2 / EX-08C1；设计 SSOT 不再保留 `/api` 前缀                                                                                |
...
| ---------------------------------------------------------------------------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /projects/{projectId}/contract-handover` / `ContractHandoverSummaryView`                              | 合同承接页、移交确认页、项目总览摘要 | `effectiveContractSetSummary`、`contractBaselineValidationSummary`、`currentHandoverBaselineSummary`、`latestHandoverRebaselineSummary`、`receivablePlanInitSummary`、`contractSummarySnapshotId`、`projectionLevel`、`exportPolicy`、`allowedActions`      | by `projectId`; 最新当前有效摘要                         | 按合同 / 项目可见性与摘要投影输出，不默认暴露完整合同详情 | `query-view-boundary-design.md` §5.2  | EX-08B1 已实现；`EX-15C` 已将运行时路由收口为 `/api/projects/{projectId}/contract-handover`，并保留项目级最近再基线化选择逻辑               |
| `GET /projects/{projectId}/project-handover` / `GET /project-handovers/{id}` / `ProjectHandoverDetailView` | 移交确认页、移交追溯页、后续冻结页   | `effectiveContractSetSummary`、`contractSummarySnapshotId`、`currentHandoverBaselineSummary`、`participantConfirmationSummary`、`receiptJudgmentModeSummary`、`summaryPackageKey`、`summarySnapshotId`、`projectionLevel`、`exportPolicy`、`allowedActions` | by `handoverId` / `projectId`; 时间倒序                  | 参与角色确认区、通知、打印材料与审计摘要共享同一摘要快照  | `query-view-boundary-design.md` §5.1  | EX-08B2 已实现详情主链；`EX-15C` 已将运行时路由收口为 `/api/projects/{projectId}/project-handover` 与 `/api/project-handovers/{handoverId}` |
...
- Route -> command: canonical route 已冻结为 `POST /project-handovers/{id}:confirm`、`POST /contract-handover-rebaselines` 与 `POST /approval-summary-packages/{targetId}:review`；`EX-15C` 已将确认命令运行时路由收口到 colon-action，当前设计与实现一致。
- Query -> view: canonical query route 已冻结为 `GET /projects/{projectId}/contract-handover`、`GET /projects/{projectId}/project-handover` 与 `GET /project-handovers/{id}`；`EX-15C` 已去掉原 `/contract-handover-summary`、`/project-handover-detail` 与 `/detail` page suffix，当前设计与实现一致；项目级最近再基线化选择与 `project_receipt_judgment_freeze` 当前冻结来源输出逻辑继续生效，前端不得临时拼装。
...
- Delivered Boundary:
  ...
  - 当前运行时已落地 `GET /projects/:projectId/contract-handover`、`GET /projects/:projectId/project-handover`、`GET /project-handovers/:handoverId`、`POST /project-handovers/:handoverId:confirm` 与 `POST /contract-handover-rebaselines`；其中 route canonical cutover 由 `EX-15C` 完成。
