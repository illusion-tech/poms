...
- 触发原因: `IdentityProviderScopeListSchema` 将 OAuth scope 列表限制为最多 32 项，但 `searchScopes` 曾单独允许 32 项，飞书用户搜索还会自动注入 `contact:user:search`。当 token 不回显 scope 时，grant fallback 可能生成并持久化 33 项，违反 shared contract / OpenAPI 上限。初次纠偏后，G3 review 又发现 `toOAuthGrantSummary()` 会直接回传历史 `grant.scopes`，旧数据或绕过写入的异常 snapshot 仍可能突破相同契约。
- 本次目标:
  ...
  - 在 grant summary 读侧与用户搜索前统一规范化、验证历史 snapshot；无法信任的 snapshot 只能触发重新授权，不能截断或伪造成已授予范围。
...
- 本次纠偏后可恢复的可信边界: 所有写入和 API 返回的 grant scope 快照均满足 32 项、单项 128 字符的 shared contract；异常历史 snapshot 不会被传给客户端或飞书搜索调用，Admin 能得到重新授权原因。
...
| Query boundary            | `IdentityProviderService`                                    | config write / authorize / summary / search | Active | 服务端是最终容量防线。                            |
...
| DTO / contract naming     | `IdentityProviderScopeListSchema` 已定义 32 / 128 限制，但写入和历史读侧曾未统一执行。 | 导出并复用上限常量；summary 不输出违规 snapshot，不改 schema shape。              |
...
| Status machine            | 容量超限是结构化配置输入错误，不是管理员可选择的生命周期状态。                         | Admin 预检阻止提交；历史 snapshot 在 summary 中降级为重新授权诊断，在搜索前阻断。 |
...
- Blocking Findings:
  ...
  4. 历史 grant snapshot 可绕过写侧验证并由 summary 原样返回。
...
- 本批修复范围:
  ...
  3. grant summary 读取和用户搜索前的历史 snapshot 规范化 / 保护；异常 snapshot 返回重新授权诊断且不调用飞书。
  4. API/Admin focused tests 与本 checkpoint、tracker、progress 回写。
- 本批未修复范围:
  1. 不替代飞书开放平台的权限审批、实际 scope 授予或 provider 侧限制。
  2. 不批量修复已在生产数据库中存在的违规 grant；summary 不会泄露违规值，用户搜索会显式拒绝并要求重新授权。
...
| 历史 grant scope snapshot | summary 原样返回，搜索仅检查 required scope | 读取统一规范化；违规值不进入 summary，搜索前拒绝并记录重新授权诊断 | Pass   |
| Admin 配置体验            | 无有效数量提示，依赖通用保存失败            | 显示最终预算、超限提示并解析结构化容量错误                         | Pass   |

### 5.1 2026-07-10 Follow-up Corrective Record

- Drift classification: `new-real-drift`。
- 发现位置: PR `#29` review，`toOAuthGrantSummary()` 的历史 persistence -> response 边界。
- Corrective decision: 对可规范化的空白 / 重复项返回去重后的 scope；对 distinct count 超限或单项超长的 snapshot，不截断、不回填当前请求范围，而是返回空 scope、required / missing scope 与中文重新授权诊断。用户搜索使用同一判定，异常时持久化 `lastError` 并在 token 解密或飞书调用前返回结构化 `400`。
- Why no data migration: 该异常应通过当前管理员重新授权恢复真实 provider 授权事实；批量改写历史 scope 会伪造或丢失授权证据。
...
| API focused tests                | Yes      | `corepack pnpm nx test poms-api --runInBand --testPathPatterns=identity-provider.service.spec --skip-nx-cache`          | Pass   | 49 passed; config / authorize / callback / legacy summary / search scope capacity    |
...
- 已解除的阻断: 最终 scope 总预算、provider 回传上限、历史 snapshot response / search 边界和 Admin 提交前反馈均已由本地验证覆盖。
- 仍存在的阻断:
...
- Conditions: 所有第 6 节 required checks 通过，OpenAPI/client 无非预期 drift；待 PR `#29` Copilot 复审确认无未解决 scope snapshot 评论。
