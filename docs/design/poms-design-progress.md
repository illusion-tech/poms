...
- `L3` 第一切片 `EX-08`：已完成 `EX-08A0` 实施准备与 `EX-08A1` 核心 DDL；新增 `project_handover`、`contract_handover_rebaseline_record`、`handover_baseline_impact_item` 的 migration / entity / repository，并将 `project_operating_snapshot`、`period_closing_snapshot`、`operating_restatement_record` 的 `handover_rebaseline_record_id` 补齐到 `contract_handover_rebaseline_record.id` 的 FK，关闭 `EX-07A-E01 / EX-07B-E01 / EX-07C-E01 / EX-08-E01`。`contract_term_snapshot` 物理表缺口已登记为 `EX-08-E05`，需在命令 guard 消费前关闭或明确替代来源。当前 `EX-08` 仍未完成，下一步进入 `EX-08A2` 摘要快照最小承接。
