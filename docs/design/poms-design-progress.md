...
**“总纲需求与 HLD 已完成第一阶段基线收敛，关键架构决策已固定，平台治理域详细设计已完成评审前收口并进入 Review，其他模块详细设计继续拆分推进”**
...
| 模块详细设计拆分   | 已启动   | `platform-governance/` 已形成首个域级子目录，其他模块设计待继续展开   |
...
| `docs/design/poms-design-progress.md`                                     | Active                    | 当前设计进度跟踪与治理看板                                           | 是                 |
...
| `docs/design/platform-governance/platform-governance-review-summary.md`   | Active                    | 平台治理域评审结论摘要，记录本轮评审结论、已关闭阻塞项和后续动作     | 是                 |
...
| `docs/design/business-authorization-matrix.md`                            | Draft (Baseline)          | 业务对象动作授权矩阵基线，已补销售、合同资金、提成与横切审批首批动作 | 是                 |
...
| 销售流程域 | 已基本明确 | 已基本明确 | `Project` 主对象命名、ADR-011 已固定 | Draft    | 生命周期主链路与第一批对象动作已回写，下一步可继续细化合同资金与审批流输入   |
| 合同资金域 | 已基本明确 | 已基本明确 | ADR-004、ADR-007 已固定              | Draft    | 已启动合同、回款、成本、发票基线设计，下一步可继续细化审批节点与提成联动输入 |
| 提成治理域 | 已基本明确 | 已基本明确 | ADR-005、ADR-006、ADR-007 已固定     | Draft    | 已启动提成计算、发放、异常调整与重算基线设计，下一步可细化审批与授权输入     |
| 横切支撑域 | 已基本明确 | 已基本明确 | 审批、审计、附件、通知方向已稳定     | Draft    | 已启动审批流、统一待办、通知与审计基线设计，下一步可继续补齐业务授权矩阵     |
...
以下文档尚未开始，但已经具备明确的上游输入和拆分价值：

1. 继续补齐 `business-authorization-matrix.md` 的业务域详细矩阵
...
- 模块级详细设计尚未产出，接口与表结构仍不可冻结
- 业务权限矩阵已形成首版基线，但字段级权限、组织范围和复杂审批链仍未细化
...
1. 继续补齐 `business-authorization-matrix.md`

其中第一优先级建议转为继续补齐 `business-authorization-matrix.md`，因为销售流程域、合同资金域、提成治理域和横切审批流的详细设计基线已经形成，当前最合适的是把对象动作、审批权、确认权和关闭权统一回填成可执行矩阵。
...
- 每次新建模块设计文档时，更新“设计资产清单”和“各业务域进度”
