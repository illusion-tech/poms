...
2. 对提成异常调整 / 重算链补一次真实数据库迁移与接口探活回写
...
1. **P1-S10 / P1-S11 / P1-S12 已完成**：提成规则版本、角色分配、计算、生效、发放草稿、统一审批待办、异常调整、执行调整、重算替代链、项目级提成治理页与工作台待办跳转均已具备真实实现；本轮同时完成了 `CommissionAdjustment` 持久化、`suspended` 状态补齐、`decimal` 精确建模、`rowVersion` 并发控制补齐与 OpenAPI / API Client 稳定化，并已把 `Migration20260327090000_init_commission_s12` 执行到真实数据库；当前后端测试套件 14 suite / 150 测试全绿，前端生产构建通过
2. **下一步**：回到 `P1-S07` 发起第一阶段正式收口评估，而不是继续新增第一阶段功能切片
