...
1. 平台治理域后端真实化：补齐 `User`、`Role`、`OrgUnit` 及其关系模型、管理接口与测试（已启动 `P1-S08`，并落首批主表、关系表、平台基础 seed、`GET /platform/users` 真实关系聚合查询、真实平台主数据优先的 `auth/profile` 聚合、平台服务 / 控制器测试骨架、`/platform/users` 前端真实 API 接入、平台用户 + 角色 + 组织单元全部写侧命令接口、所有写侧命令完整测试覆盖，以及登录认证从 fixture 迁移至真实平台数据（bcrypt 验证 + 权限从 `role_permission_assignment` 实时计算 + migration `20260325180000` 已落库）；测试套件全绿（79 个测试，10 个 suite 均通过）；fixture 保留过渡期兜底，后续完成后可清理）
...
1. 平台导航治理闭环：导航条目与真实角色权限正式绑定（`P1-S08` 最后一个核心缺口）
2. 前端平台管理页接入真实写侧接口（用户 / 角色 / 组织 CRUD）
3. 上述完成后，发起第一阶段收口评估，再评估提成治理域启动时机
