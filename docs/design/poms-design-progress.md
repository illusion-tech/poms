...
- `L3` 第一切片 `EX-08`：已完成 `EX-08A0` 实施准备、`EX-08A1` 核心 DDL、`EX-08A2` 摘要快照最小承接、`EX-08A3` 多方确认最小承接、`EX-08B1` 合同承接摘要 query、`EX-08B2` 移交详情 query、`EX-08B3A` 移交确认 command / guard 与 `EX-08B3B0` 合同变更最小持久化前置；新增 `project_handover`、`contract_handover_rebaseline_record`、`handover_baseline_impact_item`、`approval_summary_package_definition`、`approval_summary_snapshot`、`approval_summary_field_projection`、`confirmation_record`、`confirmation_participant` 与 `contract_amendment` 的 migration / entity / service 承接，并新增 `GET /projects/:projectId/contract-handover-summary`、`GET /projects/:projectId/project-handover-detail`、`GET /project-handovers/:handoverId/detail`、`POST /project-handovers/:handoverId/confirm` 与共享 `ContractHandoverSummaryView` / `ProjectHandoverDetailView` / `ConfirmProjectHandoverRequest` / `ConfirmProjectHandoverResult` 契约。当前已关闭 `EX-07A-E01 / EX-07B-E01 / EX-07C-E01 / EX-08-E01 / EX-08-E02 / EX-08-E03`；`contract_term_snapshot` 物理表缺口仍登记为 `EX-08-E05`，最近再基线化项目级查询链限制登记为 `EX-08-E06`，回款判断模式冻结来源限制登记为 `EX-08-E07`，B3A/B3B0 不关闭这些例外。当前 `EX-08` 仍未完成，下一步进入 `EX-08B3B1` 再基线化命令 / 最近记录链与 `EX-08B3C` 物理快照 / 回款冻结来源收口。
