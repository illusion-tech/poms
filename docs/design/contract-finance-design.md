...
- `ContractTermSnapshot` 必须来源于 `签约就绪` 输出的结构化承接包和最新通过的商业放行基线，不得在合同页手工重新拼装另一套条款事实
- `ReceivablePlan` 必须基于当前有效 `ContractTermSnapshot` 初始化，不能脱离已冻结的回款条件单独录入正式计划
- 多合同项目的项目级合同事实，默认来自当前有效合同集合中全部已通过基线校验的合同快照汇总，并保留合同级钻取
- 商业放行基线差异校验的最低执行口径如下：
  - 合同金额偏离 `<= 3%` 且未跨越毛利审批带，可进入 `需复核`
  - 合同金额偏离 `> 3%`，或导致毛利率跨越 `< 10%`、`10% ~ 30%`、`>= 30%` 任一审批带，必须重审
  - 首付款比例跨越 `20%` 分界线，或绝对偏差 `> 5` 个百分点，必须重审
  - 任一税率、计税方式、税负承担方变化，必须重审
  - 任一回款节点新增 / 删除 / 重排，或节点日期偏移 `> 10` 个自然日，必须重审
  - 任何尾款 / 质保金条件或范围承诺变化，原则上均按必须重审处理
...
2. 未完成商业放行基线差异校验，或合同草案与商业放行基线存在未重审偏离，不得确认合同生效。
3. 合同未生效，不得初始化正式 `ReceivablePlan`。
4. 未通过 `签约就绪` 输出的结构化承接校验，不得初始化正式 `ReceivablePlan`。
5. 未形成有效合同台账，不得进入正式收付款与发票跟踪。
6. 未经财务确认的 `ReceiptRecord` 不得进入提成计算生效口径。
7. 已确认回款不得直接删除，只能冲销。
8. 已生效合同关键条款不得直接改写，只能通过补充协议或变更形成新快照。
9. 已纳入毛利计算的成本记录不得静默删除或重写。
10. 发票异常解除必须留痕，不得直接把异常状态覆盖成正常状态。
...
- `invoiceType` 在第二阶段至少区分 `output` 与 `input`
- 销项发票应保留 `contractId`，以支持销售合同、应收计划、回款与开票链条追溯
- 进项发票若无法唯一对应某份销售合同，可仅绑定 `projectId`，并通过成本 / 付款链保留归属说明或分摊依据
...
- 合同草案与商业放行基线的差异校验结果
- 可初始化 `ContractTermSnapshot` / `ReceivablePlan` 的结构化承接结果
- 合同草案当前命中的差异等级、复核结论与是否必须退回重审
...
- 一个 `Project` 允许并行存在多份同时履约的有效销售合同；项目级经营、移交和提成读侧默认以“当前有效合同集合”作为合同侧事实
- 项目级交易链条的最小覆盖范围至少包括：销售合同、应收计划、回款、销项发票、成本、付款、进项发票
- 销售合同、应收计划、回款和销项发票必须同时可追溯到 `Project` 与 `Contract`
- 成本、付款和进项发票至少必须归属 `Project`；若能明确归属某份销售合同，应保留 `contractId`；若不能唯一归属，应保留共享归属说明或分摊依据
- `Project` 级读侧默认展示项目汇总结果，但必须支持按合同钻取，不能把多合同项目压缩成单一“当前有效合同”
- `签约就绪` 必须输出可初始化 `ContractTermSnapshot` / `ReceivablePlan` 的结构化承接包，不能只输出文字摘要
- 合同主链必须承接最新通过的商业放行基线；若最终合同条款在金额、税率、回款条件、质保金或税务条件上偏离基线且未完成重审，不得确认合同生效或初始化正式应收计划

## 16. 当前仍待后续细化的问题

- `ReceivablePlan` 是否需要进一步细分为首付款、阶段款、尾款、质保金等固定计划类型字典
- `PayableRecord` 与 `PaymentRecord` 是否需要在后续阶段引入更正式的审批闭环
- 销项 / 进项发票在已区分方向后，是否还需要拆出更细的生命周期和状态字典
