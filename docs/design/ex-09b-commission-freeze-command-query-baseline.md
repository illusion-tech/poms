...
| Route / query / command naming | `freezeCommissionRoleAssignment` 使用 `POST /commission-role-assignments/{id}:freeze`；详情 query 使用 `GET /commission-role-assignments/{id}`；变更申请资源使用 `POST /commission-role-change-requests` 与 `GET /commission-role-change-requests/{id}` | 当前实现已与 canonical route 对齐；slash-action、`/detail` 与 project-scoped legacy route 仅保留为历史治理留痕，不再作为运行态 drift            |
