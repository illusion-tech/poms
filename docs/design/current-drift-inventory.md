...
| `D-20260513-001` | `planned-contract-drift`     | Closed | Codex | `api-route-canonical-inventory.md` `B15` 登记 `POST /auth/sessions`，旧实现曾是 `POST /auth/login` 返回 JWT。    | `EX-66C` direct cutover 已清退 Admin Web `POST /auth/login` / `accessToken`；`EX-66E` 清退 browser E2E 旧 token helper 并关闭文档漂移。                                                                  |
| `D-20260528-001` | `new-real-drift`             | Closed | Codex | `FE-63` 验收 `/attachments` 时发现 FE-49 前端聚合造成长时间不可交互；实现中曾直接新增未冻结 public route。       | 拆出 `EX-69A` corrective checkpoint，route 收口为 `GET /attachment-center-records`，并回写 route inventory / OpenAPI / generated client；focused tests、lint/build、browser smoke 与 client check 通过。 |

## 3. 本轮结论

- 当前没有开放 public route / contract drift；`D-20260528-001` 已随 `EX-69A` corrective checkpoint 关闭。
- 当前可自动修复的测试漂移已关闭。
