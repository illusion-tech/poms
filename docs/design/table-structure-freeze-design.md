...
| ----------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------- |
| `project`                           | 主体主表   | `id`、`project_code`、`project_name`、`customer_id`、`status`、`current_stage`、`owner_org_id`、`owner_user_id`                                            | 可被 `contract`、`bid_process`、`approval_record` 引用               | 第一阶段核心主链路主表；`owner*` 表达销售主责 |
| `project_owner_reassignment_record` | 动作记录表 | `id`、`project_id`、`previous_owner_org_id`、`previous_owner_user_id`、`new_owner_org_id`、`new_owner_user_id`、`reason`、`reassigned_at`、`reassigned_by` | 归属 `project`                                                       | 项目销售主责变更留痕                          |
...
| `commission_gate_review_record`                          | 动作记录表        | `id`、`binding_id`、`gate_review_decision`、`blocking_reason_code`、`summary_package_key`、`summary_snapshot_id`、`projection_level`、`export_policy`、`next_action_summary`、`handled_at`、`handled_by`、`status`                                                                                                                                                                                                                                                                                                                                   | 归属 gate 绑定结果；强关联 `approval_summary_snapshot`；可被 `commission_payout` / 审批记录引用 | gate 复核、阻断与放行留痕                        |
...
| `commission_departure_exception_decision` | 动作 / 版本表       | `id`、`project_id`、`freeze_version_id`、`version`、`is_current`、`departure_scenario_code`、`decision_code`、`decision_summary`、`confirmation_requirement_summary`、`summary_package_key`、`summary_snapshot_id`、`projection_level`、`export_policy`、`handled_at`、`handled_by`、`status`、`supersedes_id`                                                                                                                                                                                                                                                                                                                                                                                                 | 归属项目 / 当前冻结版本；强关联 `approval_summary_snapshot`                         | 离职 / 特例结论主链          |
| `commission_final_settlement_snapshot`    | 快照 / 派生表       | `id`、`project_id`、`freeze_version_id`、`gate_review_record_id`、`retention_receipt_record_id`、`departure_exception_decision_id`、`version`、`is_current`、`final_settlement_status`、`non_retention_settlement_status`、`retention_settlement_status`、`retention_requirement_summary`、`retention_receipt_summary`、`departure_exception_summary`、`baseline_selection_source`、`tax_impact_summary`、`tax_impact_pending_amount`、`data_maturity_level`、`cost_action_recommendation`、`current_action_level`、`referenced_baseline_version`、`referenced_snapshot_version`、`summary_package_key`、`summary_snapshot_id`、`projection_level`、`export_policy`、`generated_at`、`status`、`supersedes_id` | 归属项目；强关联当前冻结版本、gate review、到账事实、离职 / 特例结论与摘要快照      | 最终结算 / 质保金收口快照    |
| `commission_rule_explanation_snapshot`    | 快照 / 派生表       | `id`、`project_id`、`final_settlement_snapshot_id`、`version`、`is_current`、`current_stage_status`、`gate_decision_code`、`blocking_reason_category`、`blocking_reason_code`、`blocking_reason_summary`、`gate_decision_summary`、`next_action_summary`、`generated_at`、`status`、`supersedes_id`                                                                                                                                                                                                                                                                                                                                                                                                            | 归属项目；强关联当前 `commission_final_settlement_snapshot`                         | 统一规则解释快照             |
...
- `commission_departure_exception_decision` 必须保留同一 `summarySnapshotId` 与当前冻结版本；`stage=retention` 只能消费当前有效结论，不得从离职备注或 payout comment 临时推导。
- `commission_final_settlement_snapshot` 必须直接固定非质保 / 质保金双状态、到账 / 特例引用与统一经营依据包；最终结算、质保金结算审批、通知、打印与导出不得各自重算。
- `commission_rule_explanation_snapshot` 只固定解释所需阶段 / 决策 / 原因码与稳定摘要，不要求与页面展示结构一一同形；`allowedActions` 和其余页面拼接提示继续留给读侧聚合。
