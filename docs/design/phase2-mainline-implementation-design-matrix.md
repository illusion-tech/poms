...
| `L3` | 是               | `command/query/DTO/data model/table/schema/guard` | 已覆盖 `command/query/DTO` 的首轮下钻，并已把第三批合同变更再基线化补点写回六份实现设计总文档、关键业务主文档与联动 gate 文档                                                                                                                                     | 第一批、第三批         | 第四批中的分期移交                 | 是                       |
...
| `L5` | 是               | `command/query/DTO/data model/table/schema/guard` | 已完成第一批前置覆盖，并已把第二批 `L4 -> L5` gate 绑定与解释链补点、第三批争议处理 / 审批增强补点写回六份实现设计总文档、关键业务主文档与联动 gate 文档，同时补齐经营基线版本、税务影响摘要、快照消费与冻结结果联合追溯                                          | 第一批、第二批、第三批 | 第四批中的异常摘要增强与细粒度通知 | 是                       |
