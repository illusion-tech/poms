...
| `L2` | 是               | `command/query/DTO/data model/table/schema/guard` | 已完成第一批七层下钻，并已把第二批分摊 / 阶段 / 税务 / `as-of` 补点写回六份实现设计总文档，同时补齐移交前再基线化与执行期基线衔接                                                                                                                                 | 第一批、第二批、第三批 | 无核心后置；部分表达增强可后置     | 是                       |
...
| `L5` | 是               | `command/query/DTO/data model/table/schema/guard` | 已完成第一批前置覆盖，并已把第二批 `L4 -> L5` gate 绑定与解释链补点、第三批争议处理 / 审批增强补点写回六份实现设计总文档、关键业务主文档与联动 gate 文档                                                                                                          | 第一批、第二批、第三批 | 第四批中的异常摘要增强与细粒度通知 | 是                       |
