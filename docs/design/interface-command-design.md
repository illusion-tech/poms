...
| `Project`                    | `reassignProjectOwner`       | 变更销售主责 | 项目已创建且未关闭；目标用户与主责组织合法；请求携带变更原因和并发版本           | 受控命令  | 更新 `Project.owner*` 当前事实，并追加销售主责变更动作记录                 |
...
- `P1-S10`：优先落 `activateCommissionRuleVersion`、`freezeCommissionRoleAssignment`；冻结后争议 / 仲裁命令转入第二阶段第三批
...
| 对象                       | 允许保留的普通更新能力     | 约束摘要                                       |
| -------------------------- | -------------------------- | ---------------------------------------------- |
| `Lead`                     | 线索基础信息、销售主责维护 | 仅限未关闭、未转项目前；不得改写转化事实       |
| `Project`                  | 基础信息编辑、备注补录     | 不得改写阶段推进、成交路径、关闭结论、销售主责 |
...
| `CurrentUserProfile`       | 当前用户自助维护本人资料   | 不得混入用户名、验证标记、角色、组织关系       |
...
- `PATCH /auth/profile` 是“当前用户自助维护本人资料”的普通更新接口，不复用 `PATCH /platform/users/{id}` 的管理员语义。
- 首版自助写侧字段只包含 `displayName`、`email`、`phone`；`avatarUrl` 保持只读展示，后续若开放需另行冻结。
- 联系方式值发生变化时，写侧必须同步重置对应 `emailVerified / phoneVerified` 标记；该规则不允许交给前端本地状态推断。
