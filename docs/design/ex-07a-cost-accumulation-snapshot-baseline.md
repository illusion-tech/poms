...
- **本次明确不做**:
  ...
  - `handover_rebaseline_record_id` FK 约束 → 延迟到 EX-08（见例外 EX-07A-E01）
...
| ------------ | ----- | ----- | ----------- | ------------- | ----------- | ----- |
| EX-07A-E01 | E2 | `project_operating_snapshot.handover_rebaseline_record_id`、`period_closing_snapshot.handover_rebaseline_record_id` 的 FK 约束 | Claude | Claude | EX-08 完成后 | 字段以 `nullable uuid` 创建；`contract_handover_rebaseline_record` 表在 EX-08 落地后补加 FK；不影响本切片下游依赖 |
