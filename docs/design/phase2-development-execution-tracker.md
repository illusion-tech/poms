...
| `EX-07` | `L1/L2` | 分摊、税务影响、实时 / 期末快照、重述记录与再基线选择链落地                 | `Done` | `Codex`   | `EX-06`                   | `phase2-actual-cost-accumulation-stage-view.md`、`phase2-estimated-to-actual-cost-bridge.md`                                                       | 快照、重述、桥接规则、query、DDL、测试与文档回写完成                                | G4 = Pass 2026-04-14；EX-07A / EX-07B / EX-07C 已完成；`handover_rebaseline_record_id` FK 延迟到 EX-08 后补加（E2 例外）；`EX-08` 可基于当前冻结口径推进                                                            |
...
| `EX-07A`   | `EX-07`   | 建立分摊、税务影响、实时 / 期末快照模型                                    | `Done` | `Claude`  | `EX-06`                   | 快照与税务相关表结构完成                                                                       | G4 = Pass 2026-04-14；8 张表建模完成，migration 两阶段落地（主表结构 + 注释补丁），条件唯一索引通过 expression 函数对齐 ORM 元数据，`build` ✓ `migration-check` ✓（No changes required）；`handover_rebaseline_record_id` FK 延迟到 EX-08（E2 例外 EX-07A-E01）；commit: e0f2fd9                              |
