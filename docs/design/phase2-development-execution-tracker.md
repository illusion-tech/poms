...
| `EX-06` | `L1/L2` | 执行期成本记录对象与来源映射落地                                            | `Doing` | `Codex`   | `EX-05`                   | `phase2-project-actual-cost-records.md`、`phase2-cost-source-to-project-record-mapping.md`、`ex-06d-payable-payment-tax-semantics-baseline.md`     | 成本记录、来源映射、最小命令读写链完成，且 procurement/payment 金额税额语义收口完成 | corrective checkpoint 已收口第一批 drift；`EX-06B1 ~ EX-06B4` 已完成 `PAYMENT_FACT / INVOICE / EXPENSE / PROCUREMENT` 映射、finance-scoped 读侧、OpenAPI / client、migration-check 与 `poms-api-e2e`；当前新增 `EX-06D` 以方案 C 收口 procurement/payment 金额税额语义，完成前 EX-06 不进入最终关闭 |
...
| `EX-06D`   | `EX-06`   | `PayableRecord / PaymentRecord` 税额语义升级与统一成本金额映射重基线       | `Doing` | `Codex`   | `EX-06C`                  | DDL、entity、contract、OpenAPI、映射、测试与文档回写完成，procurement/payment 金额语义不再漂移 | 已按 `ex-06d-payable-payment-tax-semantics-baseline.md` 冻结方案 C；目标是在 EX-07 前收口 `amountExcludingTax / taxAmount / amountIncludingTax` 的单一事实源，避免继续把单金额错误写入统一成本 `amountIncludingTax`                                                   |
