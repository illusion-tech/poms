...
| `EX-04` | `GOV`   | 导航治理闭环与授权关系收口                                                  | `Done`        | `Claude`  | `EX-02`、`EX-03`          | `platform-governance/navigation-design.md`、`platform-governance/navigation-route-mapping.md`                                                      | 导航事实源、权限过滤、路由收口、测试与文档回写完成           | NAVIGATION_TREE SSOT；getNavigationForUser 权限过滤；审计快照 SHA256；单测 15 个 + E2E 42 个全通过                        |
...
| `EX-04C`   | `EX-04` | 完成导航治理测试、审计与文档回写                                           | `Done`        | `Claude`  | `EX-04B`                  | 审计、测试、路由文档、设计回写完成               | getNavigationAuditSnapshot SHA256；visibility spec（isHidden/isDisabled）；viewer 初始导航 e2e；15 单测 + 42 e2e 全通过                             |
...
| `EX-02A-01`  | `EX-02A`   | 明确 `Role` 与权限关系表职责边界             | `Design Freeze` | `Done`        | `EX-01A-01`              | 表结构清单              | 已回看 `role-permission-design.md`、冻结文档与现有 schema                                                                                                                                |
| `EX-02A-02`  | `EX-02A`   | 编写 `Role`、权限关系 migration SQL          | `Migration`     | `Done`        | `EX-02A-01`              | migration 文件          | 角色主表、关系表、索引、唯一约束已存在并完成一致性校验                                                                                                                                   |
| `EX-02A-03`  | `EX-02A`   | 回写 DDL / 执行板                            | `Docs`          | `Done`        | `EX-02A-02`              | 文档回写                | 结构文档、设计文档与 tracker 状态同步                                                                                                                                                    |
| `EX-02B-01`  | `EX-02B`   | 建立 `Role`、权限关系 entity / mapping       | `Backend`       | `Done`        | `EX-02A-02`              | entity / mapping 代码   | 实体 / mapping 与 migration 一致，无隐式漂移                                                                                                                                             |
| `EX-02B-02`  | `EX-02B`   | 实现角色列表、详情、权限绑定 command / query | `Backend`       | `Done`        | `EX-02B-01`              | command / query 代码    | 列表、详情、创建、更新、启停、权限绑定与权限字典闭环可用                                                                                                                                 |
| `EX-02B-03`  | `EX-02B`   | 补角色绑定与授权收敛测试                     | `Test`          | `Done`        | `EX-02B-02`              | 单测 / e2e              | 覆盖绑定、停用角色后的授权收敛，并通过 API / Admin E2E；设计 §12 的 `inactive/deprecated` 权限测试场景在第一阶段 N/A（权限字典为静态常量，无生命周期变更机制，待权限字典治理实现后补齐） |
| `EX-03A-01`  | `EX-03A`   | 明确 `User`、用户-角色、用户-组织关系表边界  | `Design Freeze` | `Done`        | `EX-01A-01`、`EX-02A-01` | 表结构清单              | 已对照 user-management-design.md；确认 LocalCredential 独立表为 Phase 1 必须项                                                                                                           |
| `EX-03A-02`  | `EX-03A`   | 编写用户与关系表 migration SQL               | `Migration`     | `Done`        | `EX-03A-01`              | migration 文件          | Migration20260405100000 已写入；含 local_credential 建表、历史 password_hash 迁移、email_verified / phone_verified 新增字段                                                              |
| `EX-03A-03`  | `EX-03A`   | 回写 DDL / 执行板                            | `Docs`          | `Done`        | `EX-03A-02`              | 文档回写                | tracker 已更新                                                                                                                                                                           |
