...
| `EX-06B`   | `EX-06` | 实现成本记录命令链与来源映射规则                                           | `Doing` | `Codex`   | `EX-06A`                  | 写侧链路与最小读侧链路完成                       | 第一批已修 LABOR 成本率覆盖校验、金额计算与替代链；当前下一可编码切片已收敛为 `PAYMENT_FACT <- PaymentRecord` 映射，`PROCUREMENT / INVOICE / EXPENSE` 仍因缺上游对象阻断 |
