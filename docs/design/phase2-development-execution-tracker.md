...
| `EX-06` | `L1/L2` | 执行期成本记录对象与来源映射落地                                            | `Doing` | `Cursor`  | `EX-05`                   | `phase2-project-actual-cost-records.md`、`phase2-cost-source-to-project-record-mapping.md`                                                         | 成本记录、来源映射、最小命令读写链、测试与文档回写完成       | `G3 = Block`：见 `ex-06-implementation-baseline.md`；本轮先修成本率版本链、契约类型与 LABOR 写侧，非 LABOR 来源映射仍阻断 EX-06 Done  |
...
| `EX-06A`   | `EX-06` | 建立统一成本记录对象与来源映射表结构                                       | `Doing` | `Cursor`  | `EX-05`                   | 成本记录与映射 migration 完成                    | 本轮修复 `internal_cost_rate_version` 的 `rate_key` / `version` / `status` / 当前有效约束；来源映射专表或命令仍需后续收口                                       |
| `EX-06B`   | `EX-06` | 实现成本记录命令链与来源映射规则                                           | `Doing` | `Cursor`  | `EX-06A`                  | 写侧链路与最小读侧链路完成                       | 本轮修 LABOR 成本率覆盖校验、金额计算与替代链；采购 / 发票 / 费用 / 付款事实映射仍为 `G3` 阻断项                                                                |
| `EX-06C`   | `EX-06` | 完成成本记录测试与文档回写                                                 | `Doing` | `Cursor`  | `EX-06B`                  | 测试、设计回写完成                               | 本轮补第一批修复测试；EX-06 父任务须等非 LABOR 来源映射、读侧追溯和文档回写完成后才能进入 `G4`                                                                  |
