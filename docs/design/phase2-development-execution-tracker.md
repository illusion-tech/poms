...
| `EX-06` | `L1/L2` | 执行期成本记录对象与来源映射落地                                            | `Doing` | `Codex`   | `EX-05`                   | `phase2-project-actual-cost-records.md`、`phase2-cost-source-to-project-record-mapping.md`                                                         | 成本记录、来源映射、最小命令读写链、测试与文档回写完成       | `G3 = Block`：见 `ex-06-corrective-checkpoint.md`；第一批纠偏 checkpoint 已完成 `migration-check`、API 单测、API / Admin build、OpenAPI / client 与 `poms-api-e2e` 校验；下一可编码子切片见 `ex-06-payment-fact-and-read-side-baseline.md`，仅冻结 `PAYMENT_FACT` 映射 + finance-scoped 读侧 |
...
| `EX-06B`   | `EX-06`   | 实现成本记录命令链与来源映射规则                                           | `Doing`   | `Codex`   | `EX-06A`                  | 写侧链路与最小读侧链路完成                                                   | 第一批已修 LABOR 成本率覆盖校验、金额计算与替代链；`EX-06B1`、`EX-06B2`、`EX-06B3` 已完成 `PAYMENT_FACT / INVOICE / EXPENSE` 映射与 finance-scoped 读侧闭环，剩余仅 `PROCUREMENT` 待上游对象落地    |
| `EX-06C`   | `EX-06`   | 完成成本记录测试与文档回写                                                 | `Doing`   | `Codex`   | `EX-06B`                  | 测试、设计回写完成                                                           | 第一批修复与 `EX-06B1`、`EX-06B2`、`EX-06B3` 已完成测试与文档回写，并通过 `poms-api-e2e`；后续继续收口 `PROCUREMENT` 映射与父任务剩余读侧追溯                                                       |
