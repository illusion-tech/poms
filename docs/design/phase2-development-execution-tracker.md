...
| `EX-06B`   | `EX-06`   | 实现成本记录命令链与来源映射规则                                           | `Doing`   | `Codex`   | `EX-06A`                  | 写侧链路与最小读侧链路完成                                                   | 第一批已修 LABOR 成本率覆盖校验、金额计算与替代链；`EX-06B1` 已完成 `PAYMENT_FACT <- PaymentRecord` 映射与 finance-scoped 读侧闭环，`PROCUREMENT / INVOICE / EXPENSE` 仍因缺上游对象阻断    |
| `EX-06C`   | `EX-06`   | 完成成本记录测试与文档回写                                                 | `Doing`   | `Codex`   | `EX-06B`                  | 测试、设计回写完成                                                           | 第一批修复与 `EX-06B1` 已完成测试与文档回写，并通过 `poms-api-e2e`；后续继续收口 `INVOICE / EXPENSE / PROCUREMENT` 映射与父任务剩余读侧追溯                                                 |
...
| `EX-06B2`  | `EX-06B`  | 落地成本发票事实与 `INVOICE` 映射                                          | `Todo`    | `Codex`   | `EX-06B2A`                | 发票事实主对象、映射命令、读侧、测试与文档回写完成                           | `EX-06B2A` 已完成并打通 `InvoiceRecord` 主对象、命令 / 查询链、OpenAPI、migration-check 与 `poms-api-e2e`；下一步进入统一成本映射                                                           |
