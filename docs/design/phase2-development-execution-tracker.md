...
| `EX-06B2`  | `EX-06B`  | 落地成本发票事实与 `INVOICE` 映射                                          | `Blocked` | `Codex`   | `EX-06B2A`                | 发票事实主对象、映射命令、读侧、测试与文档回写完成                           | 先依赖 `EX-06B2A` 落地 `InvoiceRecord` 主对象与最小命令 / 查询链，再进入统一成本映射                                                                                                     |
| `EX-06B2A` | `EX-06B2` | 建立 `InvoiceRecord` 成本发票事实主对象与最小命令 / 查询链                 | `Todo`    | `Codex`   | `EX-06B1`                 | `invoice_record` migration / entity / repository / API / 测试 / 文档回写完成 | 当前已补 `ex-06-invoice-fact-prerequisite-baseline.md`；该切片完成前，`EX-06B2` 不得直接编码                                                                                             |
