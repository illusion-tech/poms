...
| `EX-06B`   | `EX-06`   | 实现成本记录命令链与来源映射规则                                           | `Doing`   | `Codex`   | `EX-06A`                  | 写侧链路与最小读侧链路完成                                                   | 第一批已修 LABOR 成本率覆盖校验、金额计算与替代链；`EX-06B1`、`EX-06B2` 已完成 `PAYMENT_FACT / INVOICE` 映射与 finance-scoped 读侧闭环，剩余 `EXPENSE / PROCUREMENT` 仍待上游对象落地       |
| `EX-06C`   | `EX-06`   | 完成成本记录测试与文档回写                                                 | `Doing`   | `Codex`   | `EX-06B`                  | 测试、设计回写完成                                                           | 第一批修复与 `EX-06B1`、`EX-06B2` 已完成测试与文档回写，并通过 `poms-api-e2e`；后续继续收口 `EXPENSE / PROCUREMENT` 映射与父任务剩余读侧追溯                                                |
...
| `EX-06B3`  | `EX-06B`  | 落地费用事实与 `EXPENSE` 映射                                              | `Blocked` | `Codex`   | `EX-06B3A`                | 费用事实主对象、映射命令、读侧、测试与文档回写完成                           | 先依赖 `EX-06B3A` 落地 `ExpenseRecord` 主对象与最小命令 / 查询链，再进入统一成本映射                                                                                                        |
| `EX-06B3A` | `EX-06B3` | 建立 `ExpenseRecord` 费用事实主对象与最小命令 / 查询链                     | `Todo`    | `Codex`   | `EX-06B2`                 | `expense_record` migration / entity / repository / API / 测试 / 文档回写完成 | 当前已补 `ex-06-expense-fact-prerequisite-baseline.md`；该切片完成前，`EX-06B3` 不得直接编码                                                                                                |
