...
| Upstream implementation | `ex-07c-allocation-tax-stage-query-baseline.md`                   | EX-07C-E01                                                                                | Pass     | 继承 `handover_rebaseline_record_id` FK 延迟到 EX-08 的例外清理  |
...
| ------------------------------------- | ---------- | ------------------- | --------------------------------------------------------------------------- | -------------------- |
| `project_handover`                    | 待 EX-08A1 | 待 EX-08A1          | `table-structure-freeze-design.md` §4.1; `schema-ddl-design.md` §8.1        | 待实现               |
| `contract_handover_rebaseline_record` | 待 EX-08A1 | 待 EX-08A1          | `table-structure-freeze-design.md` §7.8; `schema-ddl-design.md` §8.10.1     | 待实现               |
| `handover_baseline_impact_item`       | 待 EX-08A1 | 待 EX-08A1          | `table-structure-freeze-design.md` §7.8; `schema-ddl-design.md` §8.10.1     | 待实现               |
...
| ---------------------------------------------------------- | ------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------- | ------------------------------------- | ---------------------------------- |
| `project_handover.contract_summary_snapshot_id`            | 合同承接摘要快照，内部 UUID                       | `uuid not null`, FK to `approval_summary_snapshot.id`       | `contractSummarySnapshotId`           | `contractSummarySnapshotId`           | 待 EX-08A1 / A2                    |
| `project_handover.effective_handover_baseline_snapshot_id` | 当前移交前有效基线快照，内部 UUID                 | `uuid not null`, FK to `contract_term_snapshot.id`          | `effectiveHandoverBaselineSnapshotId` | `effectiveHandoverBaselineSnapshotId` | 待 EX-08A1                         |
| `project_handover.summary_snapshot_id`                     | 移交确认摘要快照，内部 UUID                       | `uuid not null`, FK to `approval_summary_snapshot.id`       | `summarySnapshotId`                   | `summarySnapshotId`                   | 待 EX-08A1 / A2                    |
| `project_handover.handover_rebaseline_record_id`           | 最近一次已生效移交前再基线化记录，内部 UUID，可空 | `uuid null`, FK to `contract_handover_rebaseline_record.id` | `handoverRebaselineRecordId`          | `handoverRebaselineRecordId`          | 待 EX-08A1；同时清理 EX-07 延迟 FK |
...
| `contract_handover_rebaseline_record.status`               | 再基线化状态                                      | `varchar(32) not null`                                      | `status`                              | `resultStatus` / summary              | 待 EX-08A1                         |
...
- Migration -> entity: 待 EX-08A1 / A2 / A3 实现并通过 `migration-check`。
...
| -------------------------------- | -------- | ------------------------------------------------------------------------ | ------------------------- | ---------------------------------------------------------------- |
| Build                            | Yes      | `pnpm nx build poms-api`                                                 | 待 EX-08C2                | 后端 cross-layer 切片必跑                                        |
| Unit tests                       | Yes      | `pnpm nx test poms-api --runInBand`                                      | 待 EX-08C2                | 覆盖命令成功、阻断和查询聚合                                     |
| API / integration tests          | Yes      | service / controller specs                                               | 待 EX-08C2                | 覆盖 persistence、guard、transaction、summary chain              |
| E2E                              | Yes      | `pnpm nx run poms-api-e2e:e2e --runInBand`                               | 待 EX-08C2                | EX-08 是跨合同、摘要、确认、状态 gate 的高风险链路，默认补 E2E   |
...
| Migration / schema check         | Yes      | `pnpm nx run poms-api:migration-check`                                   | 待 EX-08A1 / A2 / A3 / C2 | 涉及新增表与补 FK，必须确认无新增 drift                          |
...
| ------------ | ----- | ------------------------------------------------------------- | ----------- | ------------- | -------------- | ---------------------------------------------------------------------------------------------------------- |
| EX-08-E01    | E2    | `EX-07` 已存在 `handover_rebaseline_record_id` 字段但 FK 延迟 | Codex       | EX-08A1 owner | EX-08A1 完成前 | EX-08A1 必须补 `contract_handover_rebaseline_record` 并回补 EX-07 三张表的 FK；不得继续后延到 EX-09        |
...
- Conditions:
  ...
  - 后续必须按 `EX-08A1 -> EX-08A2 -> EX-08A3 -> EX-08B1 -> EX-08B2 -> EX-08B3 -> EX-08C1 -> EX-08C2 -> EX-08C3` 顺序推进。
  - `EX-08A1` 必须优先清理 `EX-07` 延迟 FK 例外。
