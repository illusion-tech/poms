...
- Migration -> entity: EX-08A1 / EX-08A2 / EX-08A3 / EX-08B3B0 已通过 `migration-check`。
...
- Guard / permission: EX-08B3A 已在写侧 guard 中复用 B2 detail 覆盖合同状态、摘要快照、多方确认、执行负责人、再基线化状态与并发版本；EX-08B3B0 已补 `ContractAmendment` 生效状态与 `contractId -> projectId` 的最小 guard 输入；B3B1/B3C 继续收口再基线化命令、物理快照来源与回款判断冻结来源。
...
| -------------------------------- | -------- | ------------------------------------------------------------------------ | ------------------------- | ------------------------------------------------------------------------------------------------------- |
| Build                            | Yes      | `pnpm nx build poms-api`                                                 | EX-08B3B0 Pass 2026-04-15 | 后端 persistence/API 切片必跑                                                                           |
| Unit tests                       | Yes      | `pnpm nx test poms-api --runInBand`                                      | EX-08B3B0 Pass 2026-04-15 | 已新增 `ContractAmendmentRepository` 单测覆盖生效变更读取、按合同读取与持久化；B3A command 单测继续通过 |
...
| E2E                              | Yes      | `pnpm nx run poms-api-e2e:e2e --runInBand`                               | EX-08B3B0 Pass 2026-04-15 | 既有 E2E 已覆盖 migration-up、seeder 与模块启动；EX-08 命令主路径仍待 EX-08C2 补充                      |
...
| Migration / schema check         | Yes      | `pnpm nx run poms-api:migration-check`                                   | EX-08B3B0 Pass 2026-04-15 | 已确认 `contract_amendment` 与 rebaseline -> amendment FK 无新增 schema drift                           |
| Whitespace                       | Yes      | `git diff --check`                                                       | EX-08B3B0 Pass 2026-04-15 | 当前工作区差异通过 whitespace 检查；generated client 正式回写仍待 EX-08C1                               |
...
| EX-08-E06    | E2     | 最近再基线化项目级查询链尚未具备独立 project 归属索引         | Codex       | EX-08B3B owner | EX-08B3 完成前 | EX-08B3B0 已补 `ContractAmendment` 最小前置与 rebaseline -> amendment FK，但尚未实现项目级最近记录选择；`rebaselineContractHandover` 落地时必须补齐项目级最近记录选择或等价稳定链路 |
...
- Conditions:
  ...
  - `EX-08B1`、`EX-08B2`、`EX-08B3A` 与 `EX-08B3B0` 已完成，后续必须先推进 `EX-08B3B1 / EX-08B3C`，再进入 `EX-08C1 -> EX-08C2 -> EX-08C3`。
