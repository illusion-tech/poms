...
| -------------------------------- | -------- | ------------------------------------------------------------------------ | ----------------------- | -------------------------------------------------------------------------------------------------- |
| Build                            | Yes      | `pnpm nx build poms-api`                                                 | EX-08A3 Pass 2026-04-15 | 后端 cross-layer 切片必跑                                                                          |
| Unit tests                       | Yes      | `pnpm nx test poms-api --runInBand`                                      | EX-08A3 Pass 2026-04-15 | 已新增 `ConfirmationService` 单测覆盖发起、重复阻断、参与人确认推进与关闭语义                      |
| API / integration tests          | Yes      | service / controller specs                                               | EX-08A3 Partial         | service 层已覆盖 transaction 语义；EX-08 command/controller 场景仍待 EX-08C2                       |
| E2E                              | Yes      | `pnpm nx run poms-api-e2e:e2e --runInBand`                               | EX-08A3 Pass 2026-04-15 | 既有 E2E 已覆盖 migration-up、seeder 与现有接口不被确认表变更破坏；EX-08 命令场景仍待 EX-08C2 补充 |
...
| Migration / schema check         | Yes      | `pnpm nx run poms-api:migration-check`                                   | EX-08A3 Pass 2026-04-15 | 涉及新增表与补 FK，必须确认无新增 drift                                                            |
...
- Conditions:
  ...
  - 后续必须按 `EX-08B1 -> EX-08B2 -> EX-08B3 -> EX-08C1 -> EX-08C2 -> EX-08C3` 顺序推进。
  - `EX-08A1` 已清理 `EX-07` 延迟 FK 例外，`EX-08A2` 已清理摘要快照最小承接例外，`EX-08A3` 已清理多方确认最小承接例外；`EX-08B3` 消费移交前有效基线前必须处理 `EX-08-E05`。
