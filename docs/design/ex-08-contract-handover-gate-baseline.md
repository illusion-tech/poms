...
- Migration -> entity: EX-08A1 / EX-08A2 已通过 `migration-check`；EX-08A3 仍需在对应切片继续验证。
...
| -------------------------------- | -------- | ------------------------------------------------------------------------ | ----------------------- | ------------------------------------------------------------------------------------------------ |
| Build                            | Yes      | `pnpm nx build poms-api`                                                 | EX-08A2 Pass 2026-04-15 | 后端 cross-layer 切片必跑                                                                        |
| Unit tests                       | Yes      | `pnpm nx test poms-api --runInBand`                                      | EX-08A2 Pass 2026-04-15 | 覆盖命令成功、阻断和查询聚合                                                                     |
| API / integration tests          | Yes      | service / controller specs                                               | 待 EX-08C2              | 覆盖 persistence、guard、transaction、summary chain                                              |
| E2E                              | Yes      | `pnpm nx run poms-api-e2e:e2e --runInBand`                               | EX-08A2 Pass 2026-04-15 | 既有 E2E 已覆盖 migration-up、seeder 与现有接口不被 FK 变更破坏；EX-08 命令场景仍待 EX-08C2 补充 |
...
| Migration / schema check         | Yes      | `pnpm nx run poms-api:migration-check`                                   | EX-08A2 Pass 2026-04-15 | 涉及新增表与补 FK，必须确认无新增 drift                                                          |
...
- Conditions:
  ...
  - 后续必须按 `EX-08A3 -> EX-08B1 -> EX-08B2 -> EX-08B3 -> EX-08C1 -> EX-08C2 -> EX-08C3` 顺序推进。
  - `EX-08A1` 已清理 `EX-07` 延迟 FK 例外，`EX-08A2` 已清理摘要快照最小承接例外；`EX-08B3` 消费移交前有效基线前必须处理 `EX-08-E05`。
