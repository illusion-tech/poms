...
function installScript(input: {
    ...
    const { config, releaseId, archivePath, force, installNginx, reloadNginx } = input;
...
const releaseId = assertReleaseId(optionalStringArg(args, "release") ?? deriveReleaseId(archivePath, config.archivePrefix));
