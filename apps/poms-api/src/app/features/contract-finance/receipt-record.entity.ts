...
import { RECEIPT_RECORD_STATUSES, type ReceiptRecordStatus } from '@poms/shared-contracts';
...
export const ReceiptRecordSchema = defineEntity({
    ...
    checks: [
        {
            name: 'chk_receipt_record_status',
            expression: `"status" in (${toSqlStringList(RECEIPT_RECORD_STATUSES)})`
        }
    ],
    properties: {
        id: p.uuid().primary().defaultRaw('gen_random_uuid()'),
        contractId: () => p.manyToOne(Contract).mapToPk().fieldName('contract_id').foreignKeyName('receipt_record_contract_id_foreign').updateRule('cascade').deleteRule('cascade'),
        projectId: () => p.manyToOne(Project).mapToPk().fieldName('project_id').foreignKeyName('receipt_record_project_id_foreign').updateRule('cascade').deleteRule('cascade'),
