...
- `PlatformBootstrapSeeder`：共享测试环境可复用的平台组织、角色和权限初始化，不写业务演示数据
- `BusinessTrialSeeder`：业务试用环境专用账号和 `TRIAL-*` 演示数据初始化，账号来自未提交的本地 CSV

当前仍保留为开发期 fixture 的内容：

- 登录账号、角色、组织与导航仍由应用内 fixture 提供，待平台治理表结构落地后再迁入数据库
- `DatabaseSeeder` 仍是本地开发 / E2E 固件，不用于共享业务试用环境
