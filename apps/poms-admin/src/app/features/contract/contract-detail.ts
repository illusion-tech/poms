...
import { ApprovalStatus, AttachmentTargetType, AuthStore, ContractStore, type ContractStatus } from '@poms/admin-data-access';
...
export class ContractDetail implements OnInit, OnDestroy {
    ...
    readonly canApprove = computed(() => this.contract()?.status === 'pending-review' && this.currentApproval()?.currentStatus === ApprovalStatus.Pending && this.isCurrentApprover());
    readonly canReject = computed(() => this.contract()?.status === 'pending-review' && this.currentApproval()?.currentStatus === ApprovalStatus.Pending && this.isCurrentApprover());
    readonly canActivate = computed(() => this.contract()?.status === 'pending-review' && this.currentApproval()?.currentStatus === ApprovalStatus.Approved && this.canManageContractFinance());
    ...
    readonly actionHeadline = computed(() => {
        ...
        if (this.contract()?.status === 'pending-review' && this.currentApproval()?.currentStatus === ApprovalStatus.Pending) return '合同正在审批流转中，等待当前审批人处理';
    ...
    readonly actionDescription = computed(() => {
        ...
        if (this.contract()?.status === 'pending-review' && this.currentApproval()?.currentStatus === ApprovalStatus.Pending) return '审批入口保留在业务页面，统一待办仅作为聚合入口。';
