        ...
        <div class="flex flex-col gap-5">
            ...
            <app-admin-list-toolbar>
                <div adminToolbarStart class="grid w-full grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-[minmax(18rem,1fr)_repeat(3,minmax(10rem,12rem))_repeat(2,minmax(9rem,10rem))_auto]">
                    ...
                    <input
                        type="date"
                        class="h-10 rounded-md border border-surface-300 bg-surface-0 px-3 text-sm text-surface-900 outline-none focus:border-primary dark:border-surface-700 dark:bg-surface-900 dark:text-surface-0"
                        [ngModel]="uploadedFrom()"
                        (ngModelChange)="uploadedFrom.set($event)"
                    />
                    <input
                        type="date"
                        class="h-10 rounded-md border border-surface-300 bg-surface-0 px-3 text-sm text-surface-900 outline-none focus:border-primary dark:border-surface-700 dark:bg-surface-900 dark:text-surface-0"
...
export class AttachmentCenter implements OnInit {
    ...
    readonly categoryFilterOptions = computed<FilterOption<string>[]>(() => [{ label: '全部分类', value: ALL_FILTER_VALUE }, ...this.dictionaryStore.activeItems().map((item) => ({ label: item.name, value: item.code }))]);
    ...
    readonly uploaderFilterOptions = computed<FilterOption<string>[]>(() => {
        const names = [
            ...new Set(
                this.store
                    .records()
                    .map((record) => record.attachment.uploadedByName)
    ...
    readonly filteredRecords = computed(() => {
        ...
        return this.store.records().filter((record) => {
            ...
            return [record.attachment.displayName, record.attachment.originalName, record.attachment.uploadedByName ?? '', record.targetName, record.targetNo, this.categoryLabel(record.attachment.category)].join(' ').toLowerCase().includes(keyword);
        });
    });
    ...
    readonly attachmentMetricItems = computed<AdminMetricItem[]>(() => [
        { label: '附件数量', value: this.filteredRecords().length },
        { label: '业务对象', value: this.filteredTargetCount() },
        { label: '最终版', value: this.finalCount() },
        { label: '可预览', value: this.previewableCount() }
    ]);
