...
const STAKEHOLDER_INFLUENCE_LABELS = OpportunityStakeholderInfluenceLevelLabel as Record<OpportunityStakeholderInfluenceLevel, string>;
const STAKEHOLDER_INFLUENCE_SEVERITY = OpportunityStakeholderInfluenceLevelSeverity as Record<OpportunityStakeholderInfluenceLevel, 'success' | 'secondary' | 'warn' | 'danger' | 'info' | 'contrast'>;
const STAKEHOLDER_ACCESS_LABELS = OpportunityStakeholderAccessLevelLabel as Record<OpportunityStakeholderAccessLevel, string>;
const STAKEHOLDER_ACCESS_SEVERITY = OpportunityStakeholderAccessLevelSeverity as Record<OpportunityStakeholderAccessLevel, 'success' | 'secondary' | 'warn' | 'danger' | 'info' | 'contrast'>;
...
const CUSTOMER_PREFERENCE_SEVERITY = CustomerPreferenceSeverity as Record<CustomerPreference, 'success' | 'secondary' | 'warn' | 'danger' | 'info' | 'contrast'>;
const WIN_PROBABILITY_LABELS = WinProbabilityLevelLabel as Record<WinProbabilityLevel, string>;
const WIN_PROBABILITY_SEVERITY = WinProbabilityLevelSeverity as Record<WinProbabilityLevel, 'success' | 'secondary' | 'warn' | 'danger' | 'info' | 'contrast'>;
...
const COMPETITOR_POSITION_OPTIONS = [...(CompetitorPositionOptions as ReadonlyArray<SalesIntelligenceOption<CompetitorPosition>>)];
const CUSTOMER_PREFERENCE_OPTIONS = [...(CustomerPreferenceOptions as ReadonlyArray<SalesIntelligenceOption<CustomerPreference>>)];
const WIN_PROBABILITY_OPTIONS = [...(WinProbabilityLevelOptions as ReadonlyArray<SalesIntelligenceOption<WinProbabilityLevel>>)];
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, ButtonModule, DialogModule, InputTextModule, SelectModule, TagModule, TextareaModule, ToggleSwitchModule, WorkspaceFeedback],
...
export class SalesIntelligencePanel implements OnChanges {
    ...
    @Input() title = '销售情报';
    @Input() description = '集中查看客户联系人、决策链、竞争态势和销售发现。';
    ...
    updateStakeholderEnum<T extends 'role' | 'attitude' | 'influenceLevel' | 'accessLevel'>(field: T, value: StakeholderForm[T] | null | undefined): void {
    ...
    updateCompetitorEnum<T extends 'position' | 'customerPreference' | 'winProbability'>(field: T, value: CompetitorForm[T] | null | undefined): void {
