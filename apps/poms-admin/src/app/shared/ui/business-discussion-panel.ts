...
import { BusinessDiscussionStore, BusinessDiscussionTargetObjectType, BusinessDiscussionType } from '@poms/admin-data-access';
import { BusinessDiscussionTargetObjectTypeLabel, BusinessDiscussionTypeLabel, BusinessDiscussionTypeOptions } from '@poms/shared-contracts';
...
const DISCUSSION_TARGET_LABELS = BusinessDiscussionTargetObjectTypeLabel as Record<BusinessDiscussionTargetObjectType, string>;
const DISCUSSION_TYPE_OPTIONS = [...(BusinessDiscussionTypeOptions as ReadonlyArray<BusinessDiscussionOption<BusinessDiscussionType>>)];
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, ButtonModule, DialogModule, SelectModule, TagModule, TextareaModule, ToggleSwitchModule, WorkspaceFeedback],
...
export class BusinessDiscussionPanel implements OnChanges {
    ...
    @Input() title = '业务讨论';
    @Input() description = '沉淀项目推进判断、关键结论、风险和补充信息。';
