...
describe('AttachmentPanel', () => {
  ...
  beforeEach(async () => {
    ...
    dictionaryItems = signal([
      createDictionaryItem(DEMAND_ATTACHMENT_CATEGORY, '需求资料'),
      createDictionaryItem(COMMUNICATION_ATTACHMENT_CATEGORY, '沟通资料', 10),
    ]);
    storeMock = {
      attachments,
      ...
      uploadAttachmentVersion: jest
        .fn()
        .mockResolvedValue(
          createAttachment({ id: 'attachment-2', versionNo: 2, previousAttachmentId: 'attachment-1' }),
        ),
      ...
      loadAttachmentVersions: jest
        .fn()
        .mockResolvedValue([
          createAttachment({ versionNo: 2, isLatest: true }),
          createAttachment({ id: 'attachment-old', versionNo: 1, isLatest: false }),
        ]),
      ...
      previewAttachment: jest.fn().mockResolvedValue({
    ...
    fixture.componentRef.setInput('title', '线索附件');
    fixture.componentRef.setInput('description', '保存线索判断材料。');
  ...
  it('loads and renders attachments for the target object', () => {
    ...
    expect(fixture.nativeElement.textContent).toContain('线索附件');
    expect(fixture.nativeElement.textContent).toContain('保存线索判断材料。');
